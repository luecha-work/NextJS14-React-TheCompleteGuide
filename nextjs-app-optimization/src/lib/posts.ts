...
function initDb(): void {
  ...
  const stmt: Statement = db.prepare("SELECT COUNT(*) AS count FROM users");

  const result: StmtResult = stmt.get() as StmtResult;
...
export async function getPosts(maxNumber: number | null): Promise<any[]> {
  let limitClause = "";
...
export async function storePost(post: PostData): Promise<sql.RunResult> {
  const stmt = db.prepare(`
...
export async function updatePostLikeStatus(
  ...
  const result: StmtResult = stmt.get(userId, postId) as StmtResult;
