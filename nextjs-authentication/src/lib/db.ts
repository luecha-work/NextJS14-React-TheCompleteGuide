...
interface TrainingCount {
  count: number;
}

const trainingCountResult = db
  .prepare<[], TrainingCount>("SELECT COUNT(*) as count FROM trainings")
  .get();
const hasTrainings = (trainingCountResult?.count ?? 0) > 0;
